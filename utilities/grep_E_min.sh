...
for i in {1..200} ; do if exists_in_list "$notfinished_dir" " " $i ; then echo "scale=6; $(grep "FIRE:" ${i}/slurm-* | tail -1 | awk '{printf "%.6f", $4 }') / $(grep "Number of atoms:" ${i}/slurm-* | awk '{print $4 }') " | bc -l ; else grep -A1 "Final energy per atom is" ${i}/slurm-* | tail -1 | awk '{print $1 }' ; fi ; done > E_min_list.dat
